...
func Build(name, host, port, pwd string, config utils.Config) {
	Auth(name, host, port, pwd, func(sess *ssh.Session) {
		...
		cmdStr := fmt.Sprintf(`wget -c -q 'https://github.com/Qingluan/kcpee/releases/download/v1.4/Kcpee-linux' && chmod +x Kcpee-linux; ps aux | grep Kcpee-linux | egrep -v '(grep|egrep|wget)' | awk '{print $2}' |xargs kill -9 ;ulimit -n 4096 ;  ./Kcpee-linux -S -R  -d -k "%s" -p %d -P ss -dns; rm ./Kcpee-linux`, config.Password, config.ServerPort)
		// fmt.Println(cmdStr)
		err := sess.Run(cmdStr)
